...
public class MailConfig {
	...
	static {
        try (var input =MailConfig.class.getClassLoader().getResourceAsStream("application.properties")) {
            if (input != null) {
                PROPERTIES.load(input);// environment variable not set yet
                for (String key : PROPERTIES.stringPropertyNames()) //return the key from application.properties
                {
                    ...
                    if (value != null && value.startsWith("${") && value.endsWith("}")) {
                        ...
                        if (envValue != null) {
                            PROPERTIES.setProperty(key, envValue);//here the environment variable is set
                        } else {
